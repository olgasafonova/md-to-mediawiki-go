...
func Convert(markdownText string, config Config) string {
	...
	// For concurrent processing on large files
	if config.Concurrent && len(text) > 50000 {
		// Process independent conversions concurrently
		var wg sync.WaitGroup
		results := make([]string, 8)

		// These conversions can run in parallel as they don't depend on each other's output
		conversions := []func(string) string{
			ConvertBoldItalic,
			ConvertHeaders,
			ConvertLinks,
			ConvertCallouts,
		...
		for i, fn := range conversions {
			wg.Add(1)
			go func(idx int, convFn func(string) string) {
				defer wg.Done()
				results[idx] = convFn(text)
			}(i, fn)
		}

		wg.Wait()

		// Apply results sequentially (order matters)
		text = results[0] // ConvertBoldItalic
